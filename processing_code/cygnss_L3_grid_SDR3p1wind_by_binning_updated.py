...
def file_open_and_clean(l2_file):
    ...
    fatal_flags = np.array([0, 4, 5, 6, 7, 8, 9, 17, 18, 19])
...
def file_save(lat_axis, lon_axis, binned_vals, binned_uncs, file_date):
    ...
    os.chdir('cyg_wind_1deg_SDR3p1_PODAAC') 
    #need to make netCDF file
    nc_file = Dataset('L3_SDR3p1_wind_' + file_date + '_HOURLY.nc', 'w', format='NETCDF4')
    nc_file.close() #I close it here so if there was an issue generating I know when

    #make a group for the variables to exist in
    nc_file = Dataset('L3_SDR3p1_wind_' + file_date + '_HOURLY.nc', 'a')
...
#To run on desktop. Get rid of os.chdir and for ifile in fnames: loop and just do the following:
#dirname = '/Users/eriley/CYGNSS/SDR3p1_winds_raw_and_gridded/raw/'
#ifile    = 'cyg.ddmi.s20180801-000000-e20180801-235959.l2.wind-mss.sand265.nc'
