...
def file_open_and_clean(l2_file):
    ...
    fatal_flags = np.array([0, 4, 5, 6, 7, 8, 9, 17, 18])
...
def file_save(lat_axis, lon_axis, binned_vals, binned_uncs, file_date, dirname, product_name):
    ...
    nc_file = Dataset('L3_'+product_name + '_wind_' + file_date + '_HOURLY.nc', 'w', format='NETCDF4')
    nc_file.close() #I close it here so if there was an issue generating I know when

    #make a group for the variables to exist in
    nc_file = Dataset('L3_' + product_name + '_wind_' + file_date + '_HOURLY.nc', 'a')
...
def grid_data(vals, unc, lats, lons, time, spacing):
    lat_bins = bin_bounds(-41, 41, spacing) 
...
for ifile in fnames:
        ...
        file_save(lat_axis, lon_axis, binned_vals, binned_uncs, date_str, 'cyg_wind_1deg_sand310', 'CDRsand310')
        os.chdir(root)
        os.chdir(root + '/cyg_wind_Lev2_CDR1p2/')
